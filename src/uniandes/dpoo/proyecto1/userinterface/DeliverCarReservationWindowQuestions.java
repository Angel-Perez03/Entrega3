...
public class DeliverCarReservationWindowQuestions extends JPanel {
	...
	public DeliverCarReservationWindowQuestions(Frame parentFrame, JDialog parentDialog) {
		...
		buildReservationId();
		buildAddDriversButton();
		buildDeliverCarButton();
	}
	...
	private void buildReservationId() {
		...
		try {
			rentalIdInput = new RentalIdInput(RentalQueries.getAllRentalOfLocationIdWithStatus(UserSession.getEmployeeLocationId(), RentalStatusQueries.getRentalStatusId("reserved")),
					rentalActualInfoLabel, carActualInfoLabel);
		} catch (Exception e) {
			rentalIdInput = new RentalIdInput(new ArrayList<String>(),rentalActualInfoLabel, carActualInfoLabel);
		}
	...
	private void buildAddDriversButton() {
		searchButton = new JButton("Add Drivers");
		searchButton.addActionListener(getAddDriversActionListener());
	...
	private ActionListener getAddDriversActionListener() {
		return new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (isFieldFilled()) {
					...
					try {
						new RentalAddDriver(parentFrame, rentalIdInput.getText());
					} catch (Exception ex) {
						JOptionPane.showMessageDialog(DeliverCarReservationWindowQuestions.this,
	...
	private void buildDeliverCarButton() {
		searchButton = new JButton("DeliverCar");
		searchButton.addActionListener(getDeliverCarActionListener());
		JPanel searchButtonPanel = new JPanel();
		searchButtonPanel.setLayout(new FlowLayout(FlowLayout.LEFT));
		searchButtonPanel.add(searchButton);
		this.add(searchButtonPanel);
	...
	private ActionListener getDeliverCarActionListener() {
		return new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (isFieldFilled()) {
					...
					try {
						...
						CarQueries.changeCarStatusTo(carId, CarStatusQueries.getCarStatusId("rented"));
						RentalQueries.changeRentalStatusTo(reservationId,
								RentalStatusQueries.getRentalStatusId("rented"));
						JPanel notificationPanel = new JPanel();
						notificationPanel.add(new JLabel("Delivered!"));
						JOptionPane.showMessageDialog(parentFrame, notificationPanel, "Delivered", JOptionPane.INFORMATION_MESSAGE);
